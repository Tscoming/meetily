...
export function LegacyDatabaseImport({ isOpen, onComplete }: LegacyDatabaseImportProps) {
  ...
  const handleBrowse = async () => {
    try {
      ...
      if (dbPath) {
        ...
        setErrorMessage('No database found at selected location. Please select the MeetVoice folder, backend folder, or the database file directly.');
  ...
  const handleStartFresh = async () => {
    try {
      ...
      setImportState('success');
      toast.success(t('database.initializedStarting'));
      ...
      setErrorMessage(String(error));
      setImportState('error');
      toast.error(`${t('database.initializationFailed')}: ${error}`);
      setTimeout(() => setImportState('idle'), 3000);
    }
  };
  ...
  return (
    <Dialog open={isOpen} onOpenChange={() => {}}>
      <DialogContent className="sm:max-w-[600px]" onPointerDownOutside={(e) => e.preventDefault()}>
        <DialogHeader>
          <DialogTitle className="text-2xl">Welcome to MeetVoice!</DialogTitle>
          <DialogDescription className="text-base pt-2">
            Do you have data from a previous MeetVoice installation?
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-6 py-4">
          ...
          <div className="space-y-3">
            <p className="text-sm text-gray-600">
              Select your previous MeetVoice folder, backend directory, or database file:
            </p>
