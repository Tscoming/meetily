        ...
        <Alert variant="destructive" className="border-amber-400 bg-amber-50">
          ...
          <AlertTitle className="text-amber-900 font-semibold">
            <div className="flex items-center gap-2">
              ...
              {!hasMicrophone && !hasSystemAudio ? t('permission.permissionsRequired') : !hasMicrophone ? t('permission.microphoneRequired') : t('permission.systemAudioRequired')}
            </div>
          </AlertTitle>
          {/* Action Buttons */}
          <div className="mt-4 flex flex-wrap gap-2">
            {isMacOS && !hasMicrophone && (
              ...
              >
                ...
                {t('permission.openMicrophoneSettings')}
              </button>
            )}
            {isMacOS && !hasSystemAudio && (
              ...
              >
                ...
                {t('permission.openScreenRecordingSettings')}
              </button>
            )}
            <button
              onClick={onRecheck}
          ...
          <AlertDescription className="text-amber-800 mt-2">
            ...
            {!hasMicrophone && (
              <>
                <p className="mb-3">
                  {t('permission.microphoneMissing')}
                </p>
                <div className="space-y-2 text-sm mb-4">
                    <p className="font-medium">{t('permission.pleaseCheck')}</p>
                    <ul className="list-disc list-inside ml-2 space-y-1">
                    <li>{t('permission.microphoneCheckConnected')}</li>
                    <li>{t('permission.microphoneCheckPermission')}</li>
                    <li>{t('permission.microphoneCheckExclusive')}</li>
                  </ul>
                </div>
              </>
            )}
            ...
            {!hasSystemAudio && (
              <>
                <p className="mb-3">
                  {hasMicrophone
                    ? t('permission.systemAudioMissingPartial')
                    : t('permission.systemAudioMissingAlso')}
                </p>
                {isMacOS && (
                  <div className="space-y-2 text-sm mb-4">
                    <p className="font-medium">{t('permission.enableSystemAudioMac')}</p>
                    <ul className="list-disc list-inside ml-2 space-y-1">
                      <li>{t('permission.installVirtualDevice')}</li>
                      <li>{t('permission.grantScreenRecording')}</li>
                      <li>{t('permission.configureAudioRouting')}</li>
                    </ul>
                  </div>
                )}
              </>
