...
export function BuiltInModelManager({ selectedModel, onModelSelect }: BuiltInModelManagerProps) {
    ...
    const setupListener = async () => {
      unlisten = await listen('builtin-ai-download-progress', (event: any) => {
        ...
        if (status === 'completed') {
          ...
          // Refresh models list
          fetchModels();
          toast.success(`${model} ${t('model.downloaded')}`);
        }
  ...
  const downloadModel = async (modelName: string) => {
    try {
      ...
      toast.error(`${t('model.downloadFailed')} ${modelName}`);
  ...
  const cancelDownload = async (modelName: string) => {
    try {
      ...
      toast.info(`${modelName} ${t('model.downloadCancelled')}`);
      setDownloadingModels((prev) => {
        const newSet = new Set(prev);
        newSet.delete(modelName);
        return newSet;
  ...
  const deleteModel = async (modelName: string) => {
    try {
      ...
      toast.error(`${t('model.failedDelete')} ${modelName}`);
    }
  };
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-4">
        <h4 className="text-sm font-bold">{t('model.builtinModels')}</h4>
      </div>
                    ...
                    {(isError || isCorrupted) && (
                      <p className="mb-1 text-xs text-red-600">
                        {isError && typeof model.status === 'object' && 'Error' in model.status
                          ...
                          : isCorrupted
                          ? t('model.corruptedDescription')
                          : t('model.genericError')}
                      </p>
                    )}
              ...
              {modelIsDownloading && progress !== undefined && (
                <div className="mt-3 pt-3 border-t border-gray-200">
                  <div className="flex items-center justify-between mb-1">
                    <span className="text-sm font-medium text-gray-900">{t('model.downloading')}</span>
