...
export async function showRecordingNotification(): Promise<void> {
  try {
    ...
    if (showNotification) {
      ...
      const toastId = toast.info(`🔴 ${translateUi('recording.notificationStarted')}`, {
        description: (
          <div className="space-y-3 min-w-[280px]">
            <p className="text-sm font-medium text-gray-900">
              {translateUi('recording.notificationStartedDescription')}
            </p>
            <label className="flex items-center gap-2 text-xs cursor-pointer hover:bg-blue-100 p-2 rounded transition-colors">
              ...
              />
              <span className="select-none text-gray-700">{translateUi('recording.notificationDontShowAgain')}</span>
            </label>
            <button
              onClick={async () => {
                if (dontShowAgain) {
            ...
            >
              {translateUi('recording.notificationAcknowledged')}
            </button>
          </div>
        ),
        duration: 10000,
