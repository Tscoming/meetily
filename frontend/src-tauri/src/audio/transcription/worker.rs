    ...
    tokio::spawn(async move {
        ...
        for worker_id in 0..NUM_WORKERS {
            let engine_clone = match &transcription_engine {
                ...
                TranscriptionEngine::Qwen3Asr(e) => TranscriptionEngine::Qwen3Asr(e.clone()),
                            ...
                            .await
                            {
                                Ok((transcript, confidence_opt, is_partial)) => {
                                    ...
                                    let confidence_threshold = match &engine_clone {
                                        ...
                                        TranscriptionEngine::Parakeet(_) | TranscriptionEngine::Qwen3Asr(_) => 0.0, // No confidence scores, accept all
                                    };
...
async fn transcribe_chunk_with_provider<R: Runtime>(
    ...
    match engine {
        ...
        TranscriptionEngine::Qwen3Asr(qwen3_asr_engine) => {
            let language = crate::get_language_preference_internal();

            match qwen3_asr_engine.transcribe_audio(speech_samples, language).await {
                Ok(text) => {
                    let cleaned_text = text.trim().to_string();
                    if cleaned_text.is_empty() {
                        return Ok((String::new(), None, false));
                    ...
                    info!(
                        "Qwen3-ASR transcription complete for chunk {}: '{}'",
                        chunk.chunk_id, cleaned_text
                    );
                ...
                Err(e) => {
                    error!(
                        "Qwen3-ASR transcription failed for chunk {}: {}",
                        chunk.chunk_id, e
                    );
