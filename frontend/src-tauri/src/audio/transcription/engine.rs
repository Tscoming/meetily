...
pub async fn validate_transcription_model_ready<R: Runtime>(
    ...
    match config.provider.as_str() {
        ...
        "qwen3Asr" => {
            ...
            match crate::qwen3_asr_engine::commands::qwen3_asr_validate_model_ready_with_config(app)
                .await
            {
                Ok(model_name) => {
                    info!(
                        "✅ Qwen3-ASR model validation successful: {} is ready",
                        model_name
                    );
                    Ok(())
                }
    ...
    match config.provider.as_str() {
        ...
        "qwen3Asr" => {
            ...
            match engine {
                ...
                None => Err(
                    "Qwen3-ASR engine not initialized. This should not happen after validation."
                        .to_string(),
                ),
            }
        }
