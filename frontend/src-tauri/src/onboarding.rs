...
pub async fn complete_onboarding<R: Runtime>(
    ...
    info!(
        "Completing onboarding with builtin-ai model: {}, transcription provider: {}, transcription model: {}",
        model, transcription_provider, transcription_model
    );
    ...
    let normalized_transcription_provider = match transcription_provider.as_str() {
        "parakeet" | "qwen3Asr" => transcription_provider.as_str(),
        other => {
            error!("Unsupported onboarding transcription provider: {}", other);
            return Err(format!("Unsupported transcription provider: {}", other));
        }
    };
    ...
    info!(
        ...
        normalized_transcription_provider, transcription_model
    );
    ...
    info!(
        "Onboarding completed successfully with models: summary={}, transcription_provider={}, transcription={}",
        model, normalized_transcription_provider, transcription_model
    );
    Ok(())
}
